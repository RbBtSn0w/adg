...
directory. The effective installation contains only the selected components and
skills; unselected conventional directories such as `hooks/` do not exist for an
agent to auto-load. Authoring files (`src/`, `test/`, docs, CI config, …) never
enter either packaged view.
...
- runtime projections are never copied from the source cache; ADG regenerates
  `.claude-plugin/`, `.codex-plugin/`, and Antigravity files in staging.

The lock records two hashes: `sourceHash` covers the complete cached payload and
...
When one selected component requires another component or a bootstrap skill, the
source manifest declares that closure with `selectionDependencies`. ADG expands
the selection before copying or hashing, so the lock, effective manifest, and
physical payload all describe the same installation.
...
2. Run `adg plugins update` so lock `sourceHash` and `installedHash` values reflect the new layout.
