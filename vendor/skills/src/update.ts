...
function recordSelfCliEntrypointMissing(): void {
  ...
  try {
    ...
    span.setStatus({ code: SpanStatusCode.ERROR, message: 'CLI entrypoint not found' });
  } finally {
    span.end();
  }
}

function runUpdateChild(
  args: string[],
  options: SpawnSyncOptionsWithStringEncoding
): SpawnSyncReturns<string> {
  const tracer = getTracer();
  return tracer.startActiveSpan('node', { kind: SpanKind.CLIENT }, (span) => {
    try {
      const result = spawnSync(process.execPath, args, options);
      annotateSubprocess(span, 'node', args, result);
      return result;
    } finally {
      span.end();
    }
...
export async function processWellKnownUpdates(
  ...
  for (const [baseUrl, items] of groups) {
    ...
    for (const name of result.changedSkills) {
      ...
      const spawnResult = runUpdateChild(
        selfCliArgv(cliEntry, [
          'skills',
          'add',
          baseUrl,
          '--skill',
        ...
        ]),
        {
          ...
          encoding: 'utf-8',
          env: getUpdateChildEnv(isGlobal ? 'github' : 'well-known'),
          shell: false,
        }
      );
...
export async function updateGlobalSkills(
      ...
      if (isGitHubSource) {
        ...
        if (tree) {
          ...
          for (const { name: skillName, entry } of itemsForSource) {
            ...
            // Older lockfiles stored a SHA-256 content hash. Only normalize that
            // legacy shape; arbitrary values must still be compared and updated.
            if (latestHash && /^[0-9a-f]{64}$/i.test(entry.skillFolderHash)) {
  ...
  if (healed) {
    try {
      ...
      // A failed healing write is non-fatal; the next run can retry it.
    }
  }
  ...
  if (checkable.length === 0 && skipped.length === 0) {
    if (failedSources.length > 0) {
      console.log(`${DIM}Some skill sources could not be verified.${RESET}`);
      return { successCount, failCount: failCount + failedSources.length, checkedCount };
    }
    if (!wkChanged) {
