...
| `src/detect-agent.ts` → import + `detectAgent()` | Drop the non-existent `AgentResult` import; define it locally and adapt `determineAgent()`'s `string \| false` into `{ isAgent, agent: { name } }` | The pinned `@vercel/detect-agent@0.1.0` exports only `determineAgent(): Promise<string \| false>` (no `AgentResult`). The upstream fork targeted a newer object-returning API, so the type import broke typecheck and `cachedResult.isAgent` was always `undefined` at runtime — agent auto-detection (non-interactive mode in `add`/`sync`/`remove`/`find`/`cli`) silently never fired. Adapting in `detectAgent()` keeps all consumers (`.isAgent`, `.agent.name`) unchanged. |
