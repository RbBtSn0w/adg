...
| `src/git.ts` → `createGitClient` inherited env sanitization | Remove executable/redirecting Git environment hooks before constructing the clone client; allow only ADG's fixed SSH BatchMode override | IDEs can inject `GIT_ASKPASS`, `GIT_PAGER`, and similar helpers even for users with no GitHub account. simple-git >=3.36 rejects those hooks before an anonymous public clone can run. Sanitizing them preserves non-interactive public installs without opting arbitrary parent-process commands into execution. |
